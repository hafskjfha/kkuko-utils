...
import WordExtractorApp from '@/app/manager-tool/extract/startx/StartX'
...
describe('StartX', () => {
  ...
  it('초기 렌더링이 정상적으로 되는지 확인', () => {
    ...
    expect(screen.getByText('X로 시작하는 단어 추출')).toBeInTheDocument()
  ...
  it('끝글자 입력이 정상적으로 동작하는지 확인', async () => {
    ...
    const wordStartInput = getOutsideHelpModal(()=>screen.getAllByPlaceholderText('시작글자를 입력하세요'))
    expect(wordStartInput).toBeDefined();
    await user.type(wordStartInput!, '다')

    expect(wordStartInput).toHaveValue('다')
  })
  ...
  it('파일 업로드 후 단어 추출이 정상적으로 동작하는지 확인', async () => {
    ...
    // 끝글자 입력
    const wordStartInput = getOutsideHelpModal(() =>
      screen.getAllByPlaceholderText('시작글자를 입력하세요')
    )
    await user.type(wordStartInput, 'te')
