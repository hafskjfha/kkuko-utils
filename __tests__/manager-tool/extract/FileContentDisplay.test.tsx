...
jest.mock('@/app/components/ui/scroll-area', () => ({
  ScrollArea: ({ children, ...props }: any) => <div {...props} data-testid="scroll-area">{children}</div>,
}))
...
describe('FileContentDisplay', () => {
  ...
  it('파일이 없을 때 플레이스홀더가 표시되는지 확인', () => {
    ...
    expect(screen.getByText('아직 파일이 업로드되지 않았습니다.')).toBeInTheDocument()
  ...
  it('대용량 파일에서 가상화 모드가 활성화되는지 확인', () => {
    ...
    const largeContent = Array.from({ length: 6000 }, (_, i) => `line${i}`).join('\n')
    
    render(<FileContentDisplay {...defaultProps} fileContent={largeContent} />)
    
    expect(screen.getByText('대용량 파일 - 가상화 모드 (6,000줄)')).toBeInTheDocument()
