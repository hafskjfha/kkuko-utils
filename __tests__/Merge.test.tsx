...
describe('Merge', () => {
  ...
  it('파일이 업로드되지 않았을 때 병합 버튼이 비활성화되는지 확인', () => {
    ...
    const mergeButton = getOutsideHelpModal(()=>screen.getAllByRole('button', { name: /파일 병합/i }))
  ...
  it('첫 번째 파일만 업로드되었을 때 병합 버튼이 비활성화되는지 확인', async () => {
    ...
    const testFile = new File(['test\ncontent\ndata'], 'test1.txt', { type: 'text/plain' })
  ...
  it('두 파일이 모두 업로드되면 병합 버튼이 활성화되는지 확인', async () => {
    ...
    const testFile1 = new File(['test\ncontent'], 'test1.txt', { type: 'text/plain' })
    const testFile2 = new File(['data\ntest'], 'test2.txt', { type: 'text/plain' })
  ...
  it('파일 병합이 정상적으로 동작하는지 확인 (정렬 활성화)', async () => {
    ...
    const testFile1 = new File(['zebra\napple'], 'test1.txt', { type: 'text/plain' })
    const testFile2 = new File(['banana\napple'], 'test2.txt', { type: 'text/plain' })
    ...
    // 병합 결과 확인 (중복 제거 및 정렬)
    await waitFor(() => {
      const downloadButton = getOutsideHelpModal(()=>screen.getAllByRole('button', { name: /병합된 파일 다운로드/i }))
  ...
  it('파일 병합이 정상적으로 동작하는지 확인 (정렬 비활성화)', async () => {
    ...
    // 정렬 비활성화
    const sortCheckbox = getOutsideHelpModal(()=>screen.getAllByRole('checkbox', { name: /결과 정렬/i }))
    await user.click(sortCheckbox)
  ...
  it('병합 결과가 없을 때 다운로드 버튼이 비활성화되는지 확인', () => {
    ...
    const downloadButton = screen.getByRole('button', { name: /병합된 파일 다운로드/i })
  ...
  it('첫 번째 파일 초기화가 정상적으로 동작하는지 확인', async () => {
    ...
    const file1Input = screen.getByLabelText('첫 번째 텍스트 파일')
    const testFile = new File(['test content'], 'test1.txt', { type: 'text/plain' })
  ...
  it('파일 업로드 중 로딩 상태가 표시되는지 확인', async () => {
    const user = userEvent.setup()
    render(<WordExtractorApp />)
    ...
    // 파일 업로드 시작
    user.upload(file1Input, testFile)
    
    // 로딩 상태 확인은 실제로는 매우 빠르게 완료되므로 스킵
    // 대신 업로드 완료 후 상태 확인
    await waitFor(() => {
      expect(screen.getByText('test1.txt')).toBeInTheDocument()
    })
  ...
  it('파일 크기 정보가 올바르게 표시되는지 확인', async () => {
    ...
    const file1Input = screen.getByLabelText('첫 번째 텍스트 파일')
    const testFile = new File(['test content for size check'], 'test1.txt', { type: 'text/plain' })
