  ...
  const handleDbProcess = async () => {
    ...
    for (const ww of chuckWords)
    {
        ...
        for (const data of wordsData){
            ...
            const docsId = letterDocsInfo[data.word[data.word.length - 1]]
            if (!docsId) continue
            docsLogsQuery.push({
                docs_id:docsId,
                word: data.word,
    ...
    const {data: wordThemeData, error: wordThemeError} = await supabase.from('word_themes').select('words(word),themes(name)').in('word_id',wordIds)
    ...
    const deleteWordIdChuck = chunkArray(wordIds,200);
